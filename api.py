...
def transactions (id_ : Optional[list[int]] = Query(None, title = "ids", description = "passa os ids para filtrar, apenas numeros inteiros", example = [1,5,15]), category : Optional[list[int]] = Query(None,alias= "cat", title = "categoria", description = "Passa os números das categorias para filtrar, apenas números", example = [1,3,4]) ,date_1 : Optional[str] = None, date_2:Optional[str] = None):
    d1 = datetime.strptime(date_1, "%d/%m/%Y").date() if date_1 else None
    d2 = datetime.strptime(date_2, "%d/%m/%Y").date() if date_2 else None
    list = financeiro.lista_filtro_api(id_,category,d1,d2)
    if list:
